...
bool calcSampleName (fwlite::ChainEvent &event, string &sampleName);
...
int main (int argc, char* argv[]) 
{
   ...
   // dumpSTLendl (optutl::stringVector ("inputFiles"));
   // return 0;
   ...
   for (event.toBegin(); !event.atEnd(); ++event, ++eventNumber) 
   {
      ...
      th1store::hist( sampleName + "_jettag")->Fill (numJets, numTags);
      ...
      th1store::hist(massName)->Fill (sumVertexMass);
      th1store::hist(massName + whichtag)->Fill (sumVertexMass);
   } // for event
...
void bookHistograms()
{
        ...
        ++baseIter)
   {
      ...
      th1store::add( new TH2F( histName, histName, 
                               6, -0.5, 5.5,
                               3, -0.5, 2.5) );
      for (int jet = 1; jet <= 5; ++jet)
      {
         for (int tag = 1; tag <= 2; ++tag)
         {
            ...
            th1store::add( new TH1F( histName, histName, 40, 0, 10) );
